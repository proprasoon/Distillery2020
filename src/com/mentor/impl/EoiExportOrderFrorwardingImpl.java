...
public class EoiExportOrderFrorwardingImpl {
	...
	public void getDetails(EoiExportOrderFrorwardingAction action){
		...
		try {
		  ...
		  "  SELECT int_app_id_f , vch_undertaking_name , importing_unit_nm , po_date FROM " +
...
public void viewImpl(EoiExportOrderFrorwardingAction act, int app_id){
	...
	try {
		...
		String query = " SELECT b.seq_pk, po_number,importing_unit_nm,po_date as  import_order_date,pi_date as validity_date,pi_number as performa_invoice_no,c.name,"+
...
public void printReport(EoiExportOrderFrorwardingAction act, EoiExportOrderFrorwardingDT dt)	
{
	...
	try {
		...
		*/ 
		...
		"  SELECT distinct (select  string_agg(g.name, ',') from licence.icd_master g  )  as icd,to_char(e.po_date, 'DD/MM/YYYY') as po_date,e.po_number,f.vch_country_name,e.country_id,e.importing_unit_nm,br.strength,                            "+
		"  dist.vch_undertaking_name,dist.vch_wrk_add,br.brand_name, pckg.package_name, b.etin, a.app_id, a.recv_date, a.brc_number,         "+

...
public ArrayList display_detail(LiquorVehicleAccidentCaseAECAction act) {
		...
		String filter = "";
	 	...
	 	if (act.getRadioType().equalsIgnoreCase("N")) {
			filter = " and verify_flg is  null and verify_dt is null and a.vch_finalize='F' ";
		} else if (act.getRadioType().equalsIgnoreCase("A")) {
			filter =  " and verify_flg is  not null and verify_dt is  not null  and a.vch_finalize='F' ";
		...
		public void rejectdata(LiquorVehicleAccidentCaseAECAction action) {
			...
			try {
				...
				queryList = 
						...
						" update   distillery.request_for_accidental_case set save_flag=null, vch_finalize=null, finalize_dt_time=null where "
					...
					if (saveStatus > 0) {
                     ...
                     if(saveStatus > 0){
                    	 ...
                    	 if (action.getVch_from().equalsIgnoreCase("CL")) {
                    			query = " UPDATE bottling_unmapped.disliry_unmap_cl SET  fl36gatepass=null, fl36_date=null  WHERE fl36gatepass='" + action.getGatepass_no()+"-Accidental" + "' " +
                    					" ";
                    			}else
                    			if (action.getVch_from().equalsIgnoreCase("FL1")) {
                    				query = "UPDATE bottling_unmapped.disliry_unmap_fl3 SET  fl36gatepass=null, fl36_date=null WHERE fl36gatepass='" + action.getGatepass_no()+"-Accidental" + "'";
                    			} else if (action.getVch_from().equalsIgnoreCase("FL1A")) {
                    				
                    				query = "UPDATE bottling_unmapped.disliry_unmap_fl3a SET  fl36gatepass=null, fl36_date=null WHERE fl36gatepass='" + action.getGatepass_no()+"-Accidental" + "' ";
                    			}
                    			
                    			if (action.getVch_from().equalsIgnoreCase("FL3")) {
                    				query = " UPDATE bottling_unmapped.disliry_unmap_fl3 SET  fl11gatepass=null, "
                    						+ " fl11_date=null WHERE fl11gatepass='"+ action.getGatepass_no()+"-Accidental"+ "' ";

                    			} else if (action.getVch_from().equalsIgnoreCase("FL3A")) {
                    				query = " UPDATE bottling_unmapped.disliry_unmap_fl3a SET  fl11gatepass=null, "
                    						+ " fl11_date=null WHERE fl11gatepass='" + action.getGatepass_no()+"-Accidental" + "' ";
                    			}
                         
                    			conn=ConnectionToDataBase.getConnection20_21();
                    			pstmt = conn.prepareStatement(query);
       						// System.out.println("first status----------------"+query);
       						saveStatus = pstmt.executeUpdate();
                     }
